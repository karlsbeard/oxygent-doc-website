...
This example demonstrates workflow in OxyGent.
...
async def note_workflow(oxy_request: OxyRequest):
    ...
    # Call time_agent directly
    oxy_response = await oxy_request.call(
        callee="time_agent",
        arguments={"query": "What time is it now"},
    )

    # Call file_agent directly
    oxy_response = await oxy_request.call(
        callee="file_agent",
        arguments={
...
oxy_space = [
    ...
    oxy.WorkflowAgent(
        ...
        func_workflow=note_workflow,   # Register workflow function
    ),
    oxy.ReActAgent(
        name="master_agent",
        is_master=True,
...
- This example shows how to use OxyGent for workflow
