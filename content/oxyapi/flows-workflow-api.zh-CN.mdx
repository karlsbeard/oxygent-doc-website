...
description: 完整 API 参考文档： Workflow flow 构造函数参数和方法
icon: Code
---
...
|参数|类型|默认值|描述|
|-----------|------|---------|-------------|
| `name` | str | 必需 | Unique identifier for the workflow flow |
| `func_workflow` | Callable | 必需 | Custom workflow function to execute |
...
- **Type**: `str`
- **必需**: Yes
- **Description**: 的唯一标识符 the workflow 在多智能体系统中. 调用时使用此名称 the workflow via `mas.call(callee="workflow_name")`.
...
- **Type**: `可选[Callable]`
- **必需**: Yes (in practice)
- **Description**: The custom workflow function to execute. Must be an async function that accepts an `OxyRequest` 和 returns a string.
...
- **Type**: `str`
- **默认**: `""`
- **Description**: A human-readable description of what this workflow does. Useful for documentation 和 debugging.
...
Your workflow function receives an `OxyRequest` 对象 包含 the following key methods:
