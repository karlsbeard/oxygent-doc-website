...
description: 完整 API 参考文档： implementing Retrieval-Augmented Generation (RAG) patterns using ReActAgent 和 knowledge injection
icon: Database
---

## 概述

RAG Agent is not a st和alone 类, but rather a design pattern implemented through `ReActAgent` combined 包含 knowledge injection functions. It improves answer accuracy 和 controllability by retrieving relevant external knowledge 和 injecting it into prompts before generating responses.
...
|参数|类型|描述|
|-----------|------|-------------|
| `oxy_request` | `OxyRequest` | Request 对象，包含 用户查询 和 context |

#### 返回值

|类型|描述|
|------|-------------|
| `OxyRequest` | Request 对象 包含 injected knowledge |

#### Implementation Points

1. **Get query**: Use `oxy_request.get_query()` to get 用户查询
...
4. **Return request**: Return modified request 对象
...
St和ard interface for retrieval functions, specific implementation depends on knowledge source.
...
|类型|描述|
|------|-------------|
| `str` | Retrieved knowledge text, multiple knowledge pieces connected 包含 newlines |
...
RAG Agent requires special prompt templates to h和le injected knowledge.
...
| `fallback_knowledge` | `str` | Fallback knowledge 当 retrieval fails |
...
3. **Format optimization**: Optimize knowledge format 和 structure
...
1. **Knowledge quality**: Ensure knowledge base quality 和 timeliness
...
2. **Format specification**: St和ardize knowledge format in prompts
3. **Priority**: Clarify usage priority of knowledge 和 tools
4. **Error h和ling**: H和le insufficient or incorrect knowledge

## Related Links

- [RAG Agent Main Documentation](/docs/agents-rag) - 概述 和 usage guide
...
- [RAG Examples](/examples/agents/rag-examples) - Practical 和使用示例
