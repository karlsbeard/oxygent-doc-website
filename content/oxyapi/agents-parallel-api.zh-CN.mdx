...
description: 完整 API 参考文档： the ParallelAgent class including constructor parameters, methods, 和 parallel execution patterns
icon: Users
---
...
ParallelAgent inherits from `LocalAgent` 和 is specifically designed for parallel task execution. It distributes requests to multiple team members (tools or sub-agents), collects all responses, 和 then uses an LLM to merge the results into a unified answer.
...
| `llm_model` | `str` | 必需 | Language model identifier for merging parallel results |
| `permitted_tool_name_list` | `list` | 必需 | List of tool/agent names to execute in parallel |
...
|参数|类型|默认值|描述|
|-----------|------|---------|-------------|
| `prompt` | `可选[str]` | Auto-generated | System prompt (automatically generated for result merging) |
| `sub_agents` | `可选[list]` | `[]` | Sub-agent list (managed through permitted_tool_name_list) |
| `tools` | `可选[list]` | `[]` | Tool list (managed through permitted_tool_name_list) |
| `short_memory_size` | `int` | 配置默认值 | Short-term memory size |
| `team_size` | `int` | `1` | Team size (ParallelAgent manages parallelism itself) |

#### Important 否te

ParallelAgent **adds no new dataclass 字段** - it inherits all parameters from LocalAgent. The key difference is in execution logic:
...
|类型|描述|
|------|-------------|
| `OxyResponse` | Combined response 包含 numbered results from all team members |
...
4. **Build merge prompt**: Creates prompt 包含 user question 和 all parallel results
...
- **Automatic merging**: LLM automatically analyzes 和 merges all parallel results
...
- `init()` - Initialize agent 和 tool lists
- `_get_history()` - Retrieve 对话历史
...
- [ParallelAgent Main Documentation](/docs/agents-parallel) - 概述 和 usage guide
...
- [Parallel Execution Examples](/examples/agents/parallel-execution) - Practical 和使用示例
