...
description: 完整 API 参考文档： PlanAndSolve flow constructor parameters 和 execution methods
icon: Code
---
...
|参数|类型|默认值|描述|
|-----------|------|---------|-------------|
| `name` | str | 必需 | Unique identifier for the flow |
...
|参数|类型|默认值|描述|
|-----------|------|---------|-------------|
| `max_replan_轮` | int | 30 | 最大数量 planning iterations |
...
| `category` | str | "flow" | Category 类ification |
...
- **Type**: `str`
- **必需**: Yes
- **Description**: Unique identifier for the PlanAndSolve flow 在多智能体系统中.
...
**Requirements:**
- Agent must be 在同一多智能体系统中注册
- Should be capable of structured output (Plan model)
- Typically uses an LLM 包含 good planning capabilities
...
- Often a ReActAgent 包含 tool access
...
- **Type**: `int`
- **默认**: `30`
- **Description**: 最大数量 planning iterations to prevent infinite loops.

**使用指南:**
- 简单任务: 5-10 轮
- Medium complexity: 10-20 轮
- Complex tasks: 20-30 轮
- Very complex: 30-50 轮 (increase 包含 caution)
...
- Can h和le unexpected situations
- Risk of circular planning (use `max_replan_轮`)

### `llm_model`
- **Type**: `str`
- **默认**: `"default_llm"`
- **Description**: LLM model name for fallback operations 当 max 轮 exceeded.
...
- **默认**: `PydanticOutputParser(output_cls=Plan)`
...
- **Type**: `PydanticOutputParser`
- **默认**: `PydanticOutputParser(output_cls=Action)`
...
1. **Initial Planning** (if `pre_plan_steps` is None):
   - Call planner agent 包含 formatted query
   - Parse response into Plan model
   - Extract list of steps

2. **Step Execution Loop**:
   - For each step in plan:
     - Format step 包含 context (past steps)
     - Call executor agent
...
4. **Termination**:
   ...
   - Max 轮 exceeded: Use LLM fallback
...
Balance completeness 和 performance:
...
| **Replanning** | 可选 | Manual | N/A |
| **Complexity** | High | Variable | Low |
| **Use case** | 多步骤任务 | Custom logic | Independent ops |
...
- [ReActAgent API](/oxyapi/agents-react-api) - Executor agent API
- [ChatAgent API](/oxyapi/agents-chat-api) - Planner agent API
