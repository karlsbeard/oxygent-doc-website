...
description: 完整 API 参考文档： ParallelFlow constructor parameters 和 execution methods
icon: Code
---
...
|参数|类型|默认值|描述|
|-----------|------|---------|-------------|
| `name` | str | 必需 | Unique identifier for the parallel flow |
| `permitted_tool_name_list` | list[str] | 必需 | List of tool/agent names to execute in parallel |
...
- **Type**: `str`
- **必需**: Yes
- **Description**: 的唯一标识符 the parallel flow 在多智能体系统中. Used 当 calling the flow via `mas.call(callee="flow_name")`.
