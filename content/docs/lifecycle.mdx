...
- **Structured Execution Flow**: Pre-process → Execute → Post-process hooks
...
└──────────────────────┘
    ↓
┌──────────────────────┐
│  _pre_save_data()    │  Save pre-execution state
│  - Store trace       │
│  - Log request       │
└──────────────────────┘
    ↓
...
└──────────────────────┘
    ↓
┌──────────────────────┐
│  _post_process()     │  Modify response
│  - Format output     │
│  - Update state      │
└──────────────────────┘
    ↓
┌──────────────────────┐
│  _post_save_data()   │  Save results
│  - Store trace       │
│  - Log response      │
│  - Update history    │
└──────────────────────┘
...
#### 1. `_pre_process(oxy_request)` → `OxyRequest`
...
#### 2. `_execute(oxy_request)` → `OxyResponse`
...
#### 3. `_post_process(oxy_response)` → `OxyResponse`
...
```python
# ✅ Good: Preserve parent behavior
async def _pre_process(self, oxy_request: OxyRequest) -> OxyRequest:
    oxy_request = await super()._pre_process(oxy_request)
    # Add custom logic
    return oxy_request

# ❌ Bad: Skip parent processing
async def _pre_process(self, oxy_request: OxyRequest) -> OxyRequest:
    # Custom logic only
    return oxy_request
```
...
```python
# ✅ Good: Register background tasks
async def start_monitoring(self):
    task = asyncio.create_task(self.monitor_loop())
    self.mas.background_tasks.add(task)
    task.add_done_callback(self.mas.background_tasks.discard)

# ❌ Bad: Orphaned background tasks
async def start_monitoring(self):
